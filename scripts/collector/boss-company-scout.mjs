...
import { buildTalentStructure } from "../build-talent-structure.mjs";
...
const DATA_ROOT = path.resolve(process.env.BOSS_BUSINESS_TALENT_BRIEF_HOME || path.join(CODEX_HOME, "boss-company-business-talent-brief"));
...
环境变量:
  BOSS_BUSINESS_TALENT_BRIEF_HOME  登录档案和输出目录，默认 ~/.codex/boss-company-business-talent-brief
...
async function saveOutputs(directory, jobs, candidate, coverage, previousSnapshot = null, entityResolution = null) {
  ...
  const talentStructure = buildTalentStructure(analysisInput);
  const headers = [
  ...
  const outputs = [
    ...
    writeJson(path.join(directory, "talent-structure.json"), talentStructure),
    atomicWrite(path.join(directory, "talent-data-panels.md"), buildTalentDataPanels(talentStructure)),
    atomicWrite(path.join(directory, "talent-attention-summary.md"), buildTalentAttentionSummary(talentStructure)),
...
export { analyze, assertCompleteDetails, compareSnapshots, main, saveOutputs, selectCompany };
